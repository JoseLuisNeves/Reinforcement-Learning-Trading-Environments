...
class stockMEnv(gym.Env):
    ...
    def reset(self):
        ...
        # wallet structure is what is going to distinguish the flexsell env from the freeflexsell one.
    ...
    def step(self, action_id):
        ...
        if action[0] == Actions.Buy.value and self.wallet[0]>=current_price: # having enough money to buy at least one unit
            self.Buy()
        elif action[0] == Actions.Sell.value and self.wallet[self.stock_id]>=v: # having enough units to satisfy asked volume
    ...
    def Buy(self):
        ...
        self.wallet[self.stock_id] += v # [volume_associated_with_entry_price_n_stock, entry_price]
        self.volume_history.append(v)

    def Sell(self, action):
        # we are still flex sell even with a free wallet, we admit asked volume for selling
        current_price = self.prices[self._current_tick]
        v = action[1]
        self.wallet[0] += current_price*v*(1-self.trade_fees)
        self.wallet[self.stock_id] -= v
        self.volume_history.append(-v)
        return current_price*v*(1-self.trade_fees)
