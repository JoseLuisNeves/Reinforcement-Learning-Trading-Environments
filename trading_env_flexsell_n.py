...
class Actions(Enum): 
    Sell = 0
    Buy = 1
    Hold = 2 #queremos dar a opção de ele não comprar nada porque senão estamos a forçar vender ou comprar sempre
...
class stockMEnv(gym.Env):
    ...
    def __init__(self, df, window_size, frame_bound, initCash, max_transactions, mode = 'train', trade_fees = 0, wallet_in_obs = False, penalty = 0):
        ...
        # in this env we want to give flexibility of volume to the sell action, so the action of selling diverges to a number of actions
        # equal to max_transactions. The total number of actions will be then that plus the action of buying and the action of holding:
    ...
    def step(self, action_id):
        ...
        # note: the buy action only has the element [1,0], given which the agent buy the max possible number of units given its money
        step_reward = 0
        if action[0] == Actions.Buy.value and self.wallet[0]>=current_price: # having enough money to buy at least one unit
            self.Buy()
        ...
        # sell function needs to check the current price, so we need to increment the time step after its called
    ...
    def Buy(self):
        current_price = self.prices[self._current_tick]
        v = self.wallet[0]//current_price 
        self.volume_history.append(v)
        #volume, we will buy as much as we can each time the buy action is selected, no unit fractions allowed 
    ...
    def Sell(self, action):
        # our portfolio units are organize by entry price. 
        # in order to not become stuck with units that give us low reward we will want to sell those first
        # making it so that the model takes responsability for its poor decision making 
        ...
        current_price = self.prices[self._current_tick]
        v = action[1]
        self.volume_history.append(-v) # if we entered this function is because we have atleast v units, so we can append it here 
        step_reward = 0
        vol_counter = 0
        ...
        while vol_counter < v:
            price = 0
            max_price_index = 0 
            # we will want to find the element with the highest entry price because it will correspond to the smallest reward
            ...
            if v-vol_counter >= self.wallet[self.stock_id][max_price_index][0]: 
                # if that element has less units then the rest of the asked volume we will take those it has, delete the element and look 
                # for the next highest entry price element
                vol_counter += self.wallet[self.stock_id][max_price_index][0]
                step_reward += (current_price - self.wallet[self.stock_id][max_price_index][1])*self.wallet[self.stock_id][max_price_index][0]*(1-self.trade_fees)
                del self.wallet[self.stock_id][max_price_index]
            else:
                # if what is left to get the total asked volume is less then what we have in this element we will take and subtrack
                # the quantity from the volume element
                vol_counter += v-vol_counter
                self.wallet[self.stock_id][max_price_index][0] -= v
                step_reward += (current_price - self.wallet[self.stock_id][max_price_index][1])*v*(1-self.trade_fees)
        self.wallet[0] += step_reward # update cash
        return step_reward

    def get_possible_actions(self):
        possible_actions = []
        possible_actions.append([Actions.Buy.value,0])
        for j in range(1,self.max_transactions+1):#e.g:we don't include buying with volume zero, in future env's that will correspond to holding 
                possible_actions.append([Actions.Sell.value,j]) #[action,volume]
